...
test('usage charging debits only the delta for an idempotent request key', async () => {
    ...
    assert.match(source, /pointsBalance:\s*\{\s*increment:\s*-chargedDelta/s);
    assert.match(source, /type:\s*'consume'/);
});
