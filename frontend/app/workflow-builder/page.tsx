...
import {
    Zap, Bot, Loader2, Sparkles, Lightbulb, Flame,
    TrendingUp, BookOpen, Swords, Globe, FlaskConical,
    FolderOpen, RefreshCw, Clock, Link, MousePointer,
    Trash2, ArrowLeft,
} from 'lucide-react';
...
const PRESET_TEMPLATES = [
    { icon: <TrendingUp size={22} />, name: '爆款打造流水线', desc: '热点→卖点→主图→评价，一站式爆款内容', gradient: 'linear-gradient(135deg, #2563eb, #60a5fa)' },
    { icon: <BookOpen size={22} />, name: '小红书内容矩阵', desc: '选题→标题→正文→评论，批量内容生产', gradient: 'linear-gradient(135deg, #dc2626, #f87171)' },
    { icon: <Swords size={22} />, name: '竞品分析报告', desc: '竞品拆解→策略分析→行动建议', gradient: 'linear-gradient(135deg, #7c3aed, #a78bfa)' },
    { icon: <Globe size={22} />, name: '跨境选品报告', desc: '市场分析→商业建议→多语翻译', gradient: 'linear-gradient(135deg, #059669, #34d399)' },
    { icon: <FlaskConical size={22} />, name: 'AI 提示词优化', desc: '提示词调试→效果评估→迭代优化', gradient: 'linear-gradient(135deg, #ea580c, #fb923c)' },
];
    ...
    return (
        <div className={styles.layout}>
            <header className={styles.header}>
                <button onClick={() => router.push('/')} className={styles.backBtn}><ArrowLeft size={16} /> 返回首页</button>
                <h1 className={styles.title}><Zap size={20} /> 工作流编排器</h1>
            ...
            <main className={styles.main}>
                    ...
                    <div className={styles.aiBox}>
                        ...
                        <div className={styles.aiContent}>
                            ...
                            <div className={styles.aiInputRow}>
                                ...
                                >
                                    {aiLoading ? <><Loader2 size={14} className="animate-spin" /> 生成中...</> : <><Sparkles size={14} /> 一键生成</>}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                {showRecs && recommendations.length > 0 && (
                    <div className={styles.recsOverlay} onClick={() => setShowRecs(false)}>
                        <div className={styles.recsModal} onClick={e => e.stopPropagation()}>
                            <h3 className={styles.recsTitle}><Lightbulb size={16} /> AI 建议创建以下智能体</h3>
                ...
                {/* Preset Templates */}
                <section>
                    <h2 className={styles.sectionTitle}><Flame size={18} /> 推荐工作流模板</h2>
                ...
                {/* My Workflows */}
                <section>
                    <h2 className={styles.sectionTitle}><FolderOpen size={18} /> 我的工作流</h2>
                    {loading ? (
                            ...
                            {myWorkflows.map(wf => (
                                ...
                                >
                                    ...
                                    <div className={styles.cardMeta}>
                                        <span><RefreshCw size={12} /> {wf.usageCount} 次运行</span>
                                        <span>
                                            {wf.triggerType === 'cron' ? <><Clock size={12} /> 定时</> :
                                                wf.triggerType === 'webhook' ? <><Link size={12} /> Webhook</> : <><MousePointer size={12} /> 手动</>}
                                        </span>
                                    </div>
                                    <button
