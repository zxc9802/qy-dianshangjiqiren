...
export function ensureVoiceServiceConfigured(): void {
    if (!VOICE_APP_KEY || !VOICE_ACCESS_KEY) {
        throw new AppError('Voice service is not configured.', 500);
    }
}

export function normalizeAudioToPcm(audioData: Buffer, fileName: string): ParsedWave {
    if (!isWave(audioData)) {
        throw new AppError(`Unsupported audio format for "${fileName}". Please upload WAV (16k/16bit/mono).`, 400);
    }

    return parseWavePcm(audioData);
}

export async function transcribeWaveBuffer(
    audioData: Buffer,
    fileName: string,
): Promise<{ text: string; diagnostics: TranscriptionDiagnostics }> {
    ensureVoiceServiceConfigured();

    const { pcmData, sampleRate, bitsPerSample, channels } = normalizeAudioToPcm(audioData, fileName);
    if (sampleRate !== PCM_SAMPLE_RATE || bitsPerSample !== PCM_BITS_PER_SAMPLE || channels !== PCM_CHANNELS) {
        throw new AppError(
            `WAV format must be ${PCM_SAMPLE_RATE / 1000}kHz / ${PCM_BITS_PER_SAMPLE}bit / ${PCM_CHANNELS}ch, got ${sampleRate}Hz / ${bitsPerSample}bit / ${channels}ch`,
            400,
        );
    }

    return transcribePcmWithByteDance(pcmData);
}
...
function parseWavePcm(audioData: Buffer): ParsedWave {
    ...
    if (!isPcm) {
        throw new AppError('WAV audio format is not PCM.', 400);
    }
    if (dataOffset < 0 || dataSize <= 0) {
        throw new AppError('WAV data chunk not found.', 400);
    }
...
function transcribePcmWithByteDance(audioData: Buffer): Promise<{ text: string; diagnostics: TranscriptionDiagnostics }> {
    return new Promise((resolve, reject) => {
            ...
            void streamAudio(ws, audioData)
                ...
                .catch((error) => {
                    if (!settled) {
                        settled = true;
                        clearTimeout(timeout);
                        reject(error instanceof Error ? error : new Error('Audio streaming failed'));
                    }
                });
        });
        ...
        ws.on('message', (raw: RawData) => {
            try {
                ...
                const message = parseServerPayload(buffer);
                ...
                // Ignore malformed ACK and heartbeat frames.
            }
        });
...
function encodeFullClientRequest(requestConfig: object): Buffer {
    const payload = Buffer.from(JSON.stringify(requestConfig), 'utf8');
...
function isFinalMessage(message: object): boolean {
    ...
    return eventName.includes('complete') || eventName.includes('finished') || eventName.includes('end');
}
