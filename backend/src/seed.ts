...
const VIDEO_BREAKDOWN_DIRECTOR_PROMPT = [
...
const BOTS = [
    ...
    { name: '视频拆解导演', slug: 'video-breakdown-director', category: '电商工具', icon: 'camera', description: '以导演、编导和剪辑的专业视角拆解视频结构、镜头语言、节奏和转化设计。', pointsPerUse: 8, sortOrder: 37 },
];

async function seed() {
    console.log('Seeding 37 bots...');

    for (const bot of BOTS) {
        let systemPrompt = getSystemPromptBySortOrder(
            bot.sortOrder,
            `你是${bot.name}，请给出专业、结构化、可执行的建议。`,
        );

        if (bot.sortOrder === 35) {
            systemPrompt = QIYA_ENTERPRISE_MANAGEMENT_PROMPT;
        } else if (bot.sortOrder === 36) {
            systemPrompt = GENERIC_CHAT_PROMPT;
        } else if (bot.sortOrder === 37) {
            systemPrompt = VIDEO_BREAKDOWN_DIRECTOR_PROMPT;
        }
    ...
    for (let i = 0; i < testCodes.length; i += 1) {
    ...
    console.log('Seed complete: 37 bots + 3 test redeem codes');
}
